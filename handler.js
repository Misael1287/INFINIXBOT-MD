...
export async function handler(chatUpdate) {
    ...
    try {
        ...
        const isAdmin = isRAdmin || user?.admin == 'admin' || false // ¿Es usuario administrador?
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (plugin.rowner && plugin.owner && !(isROwner || isOwner)) { // Todos los creadores
                    fail('owner', m, this)
                    continue
                }
                if (plugin.rowner && !isROwner) { // Creador real
                    fail('rowner', m, this)
                    continue
                }
                if (plugin.owner && !isOwner) { // Numero del creador
                    fail('owner', m, this)
                    continue
                }
                ...
                if (plugin.group && !m.isGroup) { // Solo para grupos
                    fail('group', m, this)
                    continue
                } else if (plugin.botAdmin && !isBotAdmin) { // Eres administrador
                    fail('botAdmin', m, this)
                    continue
                } else if (plugin.admin && !isAdmin) { // Usuario administrador
                    fail('admin', m, this)
                    continue
                }
                ...
                if (plugin.register == true && _user.registered == false) { // esta registrado?
                    fail('unreg', m, this)
                    continue
                }
                m.isCommand = true
...
Mensaje @${participant.split`@`[0]} detectado eliminado❗
